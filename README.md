...
12. **To manually toggle DR (Azure)**:  
...
> **Note**: Ensure the AWS CLI user (`aws configure`) or Terraform assumed role has sufficient permissions to manage **S3**, **CloudFront**, **Route 53**, and **AWS Certificate Manager**. Ensure the Azure CLI user (`az login`) or Terraform assumed role has sufficient permissions to manage **Resource Groups** and **Storage Accounts**.  
...
│   ├── main.tf                           # Main Terraform config (AWS + Azure + DNS)
│   ├── variables.tf                      # Input variables
│   ├── outputs.tf                        # Exported values
│   ├── terraform.tfvars                  # Default variable values
│   ├── providers.tf                      # AWS/Azure provider definition
